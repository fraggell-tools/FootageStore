...
## Incident History
- **2026-06-05 — Large clips silently failed to sync (`clips.file_size` int4 overflow).** Video files larger than ~2.14 GB (the int4 max) overflowed the `file_size` column, so Drive sync raised `numeric_value_out_of_range` and skipped them — logged only as `[Sync] Failed to create clip …` (the worker never surfaced the underlying Postgres error). Hundreds of clips across multiple clients were affected; the largest stored clip sat right at the int4 ceiling. **Fix:** `fileSize` → `bigint` (`mode:"number"`) in `schema.ts` + idempotent migration `0001_clip_filesize_bigint.sql`; the live column was widened with `ALTER TABLE clips ALTER COLUMN file_size TYPE bigint` after a `pg_dump` backup. Verified: backlog ingested, max `file_size` now ~20 GB. **Gotcha:** prod has **no `__drizzle_migrations` table** — the schema was built with `drizzle-kit push`, so every migration must stay idempotent (`ADD COLUMN IF NOT EXISTS`, `SET DATA TYPE`) and is safe to re-run.
- **2026-06-05 — Worker stopped draining the queue (clips stuck on "processing").** After a host-wide restart, the BullMQ consumer in `app-worker-1` wedged: 0 jobs processed in 24h, worker at 0% CPU, ~160 jobs piled up in `wait`, and 2 jobs left orphaned in `active` for ~19h — while the in-process Drive sync kept running, so new rows appeared but never left "processing". **Symptom to watch:** `docker exec app-redis-1 redis-cli llen bull:clip-processing:wait` climbing while worker CPU is 0% and there are no `[Worker] Job … completed` lines. **Fix:** `docker restart app-worker-1` — the fresh consumer reclaimed the stalled jobs and drained the backlog. No code change.
